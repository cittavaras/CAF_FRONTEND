...
const RegistroRutinas = () => {
  ...
  const handleCardioInicialChange = (event) => {
    const cardioInicial = event.target.value;
    // validation cardio Inicial
    if (cardioInicial === '' || (!isNaN(cardioInicial) && cardioInicial >= 0 && cardioInicial <= 90)) {
      setCardioInicial(cardioInicial === '' ? '' : parseInt(cardioInicial));
    }
    else {
      Swal.fire({
        icon: 'error',
        text: 'No puede ser mayor a 90 minutos',
  ...
  const handleCardioFinalChange = (event) => {
    // validation cardio final
    const cardioFinal = event.target.value;
    if (cardioFinal === '' || (!isNaN(cardioFinal) && cardioFinal >= 0 && cardioFinal <= 90)) {
      setCardioFinal(cardioFinal === '' ? '' : parseInt(cardioFinal));
    }
    else {
      Swal.fire({
        icon: 'error',
  ...
  const handleCalentamientoChange = (event) => {
    const calentamiento = event.target.value;
    //validacion calentamiento
    if (calentamiento.length < 40) {
      setCalentamiento(calentamiento);
    }
    else {
      Swal.fire({
        icon: 'error',
        text: 'La descripción es muy larga',
        confirmButtonColor: 'rgb(158 173 56)',
      });
      setCalentamiento('');
    }
  ...
  const handleVueltaALaCalmaChange = (event) => {
    // validation Vuelta a la calma
    const vamoACalmarnos = event.target.value;
    if (vamoACalmarnos.length < 40) {
      setVueltaALaCalma(vamoACalmarnos);
    }
    else {
      Swal.fire({
        icon: 'error',
  ...
  const handleNomRutina = (event) => {
    // validacion nombre
    const nameRutine = event.target.value;
    if (nameRutine.length < 30) {
      setNomRutina(nameRutine);
    }
    else {
      Swal.fire({
        icon: 'error',
  ...
  const handleAddDias = (day) => {
    ...
    //   Swal.fire({
    //     icon: 'error',
    //     text:'ta mal, no se puede, lo siento'})
  ...
  // Registro de Rutinas para el alumno

  const validarDatosEjercicio = (ejercicio) => {
    if (!ejercicio.nombre || ejercicio.nombre.trim() === '') {
      return false;
    }
    if (isNaN(ejercicio.repeticiones) || ejercicio.repeticiones <= 0) {
      return false;
    }
    if (isNaN(ejercicio.peso) || ejercicio.peso <= 0) {
      return false;
    }
    if (isNaN(ejercicio.series) || ejercicio.series <= 0) {
      return false;
    }
    if (isNaN(ejercicio.descanso) || ejercicio.descanso < 0) {
      return false;
    }
    return true;
  };
  ...
  const registrarRutina = async (e) => {
    ...
    if (!nomRutina) {
      Swal.fire({
        icon: 'error',
        text: 'La rutina no tiene nombre',
    ...
    if (exercisesInvalid) {
      ...
      Swal.fire({
        icon: 'error',
        text: 'Verifica los datos de los ejercicios. Los campos tienen requisitos específicos.',
        confirmButtonColor: 'rgba(158 173 56)',
      });
      return;
    }
    else if (!cardioInicial) {
      Swal.fire({
        icon: 'error',
        text: 'Ingrese un valor para cardio Inicial',
        confirmButtonColor: 'rgba(158 173 56)'
      });
      return;
    }
      ...
      Swal.fire({
        icon: 'error',
        text: 'Ingrese un valor para cardio Final',
        confirmButtonColor: 'rgba(158 173 56)'
      });
      return;
    }
    else if (!calentamiento) {
      Swal.fire({
        icon: 'error',
        text: 'Ingrese una descripción para el calentamiento',
        confirmButtonColor: 'rgba(158 173 56)'
      });
      return;
    }
    else if (!vueltaALaCalma) {
      Swal.fire({
        icon: 'error',
        text: 'Ingrese una descripción para mantener la calma',
        confirmButtonColor: 'rgba(158 173 56)'
      });
      return;
    }
      ...
      Swal.fire({
        icon: 'error',
        text: 'Seleccione un dia',
        confirmButtonColor: 'rgba(158 173 56)'
      });
      return;
    }
    //Cuando un estudiante registra sus rutinas 
    else {
      if (alumno.tipoUsuario === "Alumno") {
        try {
          ...
          Swal.fire({
            icon: 'success',
            text: 'Rutina registrada correctamente',
            confirmButtonColor: 'rgb(158 173 56)',
          });
      ...
      //Cuando un Instructor registra rutinas al estudiante
      else {
        try {
          const res = await axios.post(baseURL + '/rutinas/alumno/', {
            nombre: nomRutina,
  ...
  const editarRutina = async (e) => {
    ...
    //   }
    // })
    // if (!nomRutina || selectedDays.length === 0 || !cardioInicial || !cardioFinal || !vueltaALaCalma|| !calentamiento) {
    //   Swal.fire({
    //     icon:'error',
  ...
  // useEffect(() => {
  //   //console.log("selectedDayFromGet", selectedDayFromGet);
  // }, [selectedDayFromGet]);
    ...
    <>
      ...
      <div style={{ margin: '20px', marginBottom: '100px' }}>
        ...
        <Row className="form-registro-rutinas">
          ...
          <Col xs={12} className="BoxRutina mb-2">
            <TextField
              autoComplete="off"
              sx={style}
              type="text"
          ...
          <Col xs={6} className="BoxRutina">
            ...
            <TextField
              ...
              className="input-group-box-rutina"
              label="ej: 10 minutos"
              value={cardioInicial === '' ? '' : cardioInicial}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '20px' }}>
          <Button autoFocus style={{ background: 'rgb(158 173 56)' }} color="success" variant="contained" onClick={rutinaId ? editarRutina : registrarRutina}>
